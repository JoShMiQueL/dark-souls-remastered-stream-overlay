...
//
// The JS reads ?stat= params from the URL, builds DOM elements matching the
// same id/class structure as before, and updates values on WebSocket data.
// Adding a new stat to StatRegistry.h is enough — no HTML changes needed.
// ---------------------------------------------------------------------------
std::string WebSocketServer::GetHTMLPage() {
    ...
    std::string js =
        "<script>\n"
        + defs + defaults +
        // Parse ?stat= params client-side (preserves order, deduplicates)
        "function gp(){"
          ...
          "for(var i=0;i<p.length;i++){"
            "var kv=p[i].split('=');"
            "if(kv[0]==='stat'&&kv[1]&&r.indexOf(kv[1])===-1)r.push(kv[1]);"
          "}"
          "return r;"
        "}\n"
        ...
        "}\n"
        "function upd(d){for(var k in d){if(k==='playTime')set(k,ft(d[k]));else set(k,d[k]);}}\n"
