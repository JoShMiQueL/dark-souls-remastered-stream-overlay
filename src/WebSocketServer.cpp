...
std::string WebSocketServer::GetHTMLPage() {
    ...
    std::string js =
        ...
        "function gp(){"
          ...
          "return r;"
        "}\n"
        // Get custom label or template for a stat
        "function getParam(key){"
          ...
          "for(var i=0;i<p.length;i++){"
            "var kv=p[i].split('=');"
            "if(kv[0]===key)return kv[1]?decodeURIComponent(kv[1]):'';"
          "}"
          "return null;"
        "}\n"
        // Check if raw mode is enabled
        "function isRaw(){return getParam('mode')==='raw';}\n"
        // Get global template or stat-specific template
        "function getTemplate(key){"
          "var t=getParam('template_'+key);"
          "if(t)return t;"
          "return getParam('template');"
        "}\n"
        // Apply template with variable substitution
        "function applyTemplate(tmpl,data,statKey){"
          "var result=tmpl;"
          "var val=data[statKey];"
          "var d=S[statKey];"
          "var maxVal=d.p?data[d.p]:null;"
          "result=result.replace(/{value}/g,val);"
          "if(maxVal!==null)result=result.replace(/{max}/g,maxVal);"
          "for(var k in data){"
            "result=result.replace(RegExp('{'+k+'}','g'),data[k]);"
          "}"
          "return result;"
        "}\n"
        // Build DOM elements for each requested stat
        "function init(){"
          "var st=gp();if(st.length===0)st=D;"
          "var b=document.body;"
          "var raw=isRaw();"
          "for(var i=0;i<st.length;i++){"
            "var k=st[i],d=S[k];"
            "if(!d)continue;"
            "var tmpl=getTemplate(k);"
            "if(raw){"
              "var v=document.createElement('span');"
              "v.id=k;v.textContent='-';"
              "b.appendChild(v);"
            "}else if(tmpl){"
        ...
        "function upd(d){"
          ...
          "for(var i=0;i<st.length;i++){"
            ...
            "if(raw){"
              "set(k,d[k]);"
            "}else if(tmpl){"
              "var e=document.getElementById('stat-'+k);"
              "if(e)e.textContent=applyTemplate(tmpl,d,k);"
            "}else{"
