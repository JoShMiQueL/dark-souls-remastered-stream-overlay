...
std::string WebSocketServer::GetHTMLPage() {
    ...
    std::string js =
        ...
        // Order matters: backslash must be first to avoid partial matches
        "function processEscapes(str){"
          "return str"
            ".replace(/\\\\\\\\/g,'__DST_BACKSLASH__')"
            ".replace(/\\\\n/g,'__DST_NEWLINE__')"
            ".replace(/\\\\t/g,'__DST_TAB__')"
            ".replace(/\\\\\\|/g,'__DST_PIPE__');"
        "}\n"
        "function unprocessEscapes(str){"
          "return str"
            ".replace(/__DST_NEWLINE__/g,'\\n')"
            ".replace(/__DST_TAB__/g,'\\t')"
            ".replace(/__DST_PIPE__/g,'|')"
            ".replace(/__DST_BACKSLASH__/g,'\\\\');"
        "}\n"
        ...
        "function init(){"
          ...
          "if(lines.length===0){"
            ...
            "}"
          "}else{"
            // Cache processed templates to avoid re-parsing on every update
            "cachedTemplates=[];"
            "for(var i=0;i<lines.length;i++){"
              ...
              "for(var j=0;j<parts.length;j++){"
                ...
                "div.textContent='-';"
                "document.body.appendChild(div);"
                "lineTemplates.push({id:'line-'+i+'-'+j,template:unprocessed});"
              "}"
              "cachedTemplates.push(lineTemplates);"
            "}"
          "}"
        "}\n"
        // Update helpers
        ...
        "function upd(d){"
          ...
          "}else{"
            ...
            "for(var i=0;i<cachedTemplates.length;i++){"
              "var lineTemplates=cachedTemplates[i];"
              "for(var j=0;j<lineTemplates.length;j++){"
                "var tmpl=lineTemplates[j];"
                "var e=document.getElementById(tmpl.id);"
                "if(e)e.textContent=applyTemplate(tmpl.template,d);"
              "}"
            "}"
          "}"
        "}\n"
